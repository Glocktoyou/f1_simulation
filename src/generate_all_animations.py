"""
F1 Lap Animation Generator - All Tracks
Creates animated GIFs for Silverstone, Monaco, and Spa

Dependencies: pip install matplotlib numpy pandas pillow scipy
"""
...
import pandas as pd
from f1_real_tracks import F1Vehicle, create_silverstone, create_monaco, create_spa, simulate_real_track
...
def animate_lap(track, telemetry, lap_time, output_file='lap_animation.gif', fps=20):
    ...
    print(f"  Generating track coordinates for {track.name}...")
...
def main():
    """Generate lap animations for all tracks"""
    
    print("=" * 60)
    print("🏎️  F1 LAP ANIMATION GENERATOR - ALL TRACKS")
    print("=" * 60)
    ...
    for track_id, track in tracks:
        ...
        print(f"   Record: {track.record_lap_time:.3f}s ({track.record_holder})")
        
        print("   Simulating lap...")
        telemetry, lap_time = simulate_real_track(vehicle, track)
        
        # Add missing telemetry columns if needed
        if 'throttle' not in telemetry.columns:
            # Estimate throttle based on acceleration
            telemetry['throttle'] = 0.8  # Default throttle
        if 'brake' not in telemetry.columns:
            telemetry['brake'] = 0.0
        if 'lateral_g' not in telemetry.columns:
            telemetry['lateral_g'] = 0.0
        if 'longitudinal_g' not in telemetry.columns:
            telemetry['longitudinal_g'] = 0.0
        
        # Calculate G-forces from velocity changes
        velocities = telemetry['velocity'].values / 3.6  # convert to m/s
        times = telemetry['time'].values
        distances = telemetry['distance'].values
        
        # Longitudinal G (acceleration/deceleration)
        long_g = np.zeros(len(velocities))
        for i in range(1, len(velocities)):
            dt = times[i] - times[i-1]
            if dt > 0:
                accel = (velocities[i] - velocities[i-1]) / dt
                long_g[i] = accel / 9.81
        telemetry['longitudinal_g'] = long_g
        
        # Lateral G (estimate from segment type)
        lat_g = np.zeros(len(velocities))
        for i, row in telemetry.iterrows():
            seg = track.get_segment_at_distance(row['distance'])
            if seg['radius'] != np.inf:
                v = row['velocity'] / 3.6
                lat_g[i] = (v**2) / (abs(seg['radius']) * 9.81)
        telemetry['lateral_g'] = lat_g
        
        # Estimate throttle/brake from longitudinal G
        telemetry['throttle'] = np.clip(telemetry['longitudinal_g'] / 2 + 0.5, 0, 1)
        telemetry['brake'] = np.clip(-telemetry['longitudinal_g'] / 4, 0, 1)
        
        print(f"   Lap time: {lap_time:.3f}s")
        print(f"   Telemetry points: {len(telemetry)}")
        
        output_file = f'images/lap_animation_{track_id}.gif'
    ...
    print("=" * 60)
    print("\nGenerated files:")
    print("  - images/lap_animation_silverstone.gif")
    print("  - images/lap_animation_monaco.gif")
    print("  - images/lap_animation_spa.gif")
